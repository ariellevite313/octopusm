...
const FLOWS = ["prediction", "pools", "launch", "listing"] as const;
...
const DEFAULT_MANUAL = {
  ...
  txSignature: "",
  flow: "launch" as typeof MANUAL_FLOWS[number],
};
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-wrap gap-2">
            {STATUS_FILTERS.map((f) => (
              <Link key={f} href={buildHref(f, currentFlow)} onClick={changeFilter}
          ...
          <div className="flex flex-wrap gap-2">
            {(["all", ...FLOWS] as const).map((f) => (
              <Link key={f} href={buildHref(currentFilter, f)} onClick={changeFilter}
                ...
                {f === "all" ? "All flows" : flowLabel(f)}
              </Link>
            ))}
          </div>
        </div>
        <Button size="sm" onClick={() => { setShowManual(true); setManualError(""); }}
      ...
      <Dialog open={showManual} onOpenChange={(o) => { if (!o) { setShowManual(false); setManual(DEFAULT_MANUAL); } }}>
        <DialogContent className="max-w-md border-border">
          <DialogHeader>
            ...
            <DialogDescription>Record a payment confirmed outside the platform.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <div>
              ...
              <input className="w-full rounded-xl border border-border bg-card px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400"
                placeholder="Wallet address" value={manual.userWallet}
            ...
            <div>
              ...
              <input className="w-full rounded-xl border border-border bg-card px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400"
                placeholder="Payment description" value={manual.title}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" min="0" step="0.01"
                  ...
                  placeholder="0.00" value={manual.amount}
              ...
              <div>
                ...
                <select className="w-full rounded-xl border border-border bg-card px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400"
                  value={manual.token} onChange={(e) => setManual((m) => ({ ...m, token: e.target.value as "usdc" | "clawdtrust" }))}>
            ...
            <div>
              ...
              <select className="w-full rounded-xl border border-border bg-card px-3 py-2 capitalize focus:outline-none focus:ring-2 focus:ring-orange-400"
                value={manual.flow} onChange={(e) => setManual((m) => ({ ...m, flow: e.target.value as typeof MANUAL_FLOWS[number] }))}>
                {MANUAL_FLOWS.map((f) => <option key={f} value={f}>{f}</option>)}
              </select>
            </div>
            <div>
              ...
              <input className="w-full rounded-xl border border-border bg-card px-3 py-2 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-orange-400"
                placeholder="Transaction hash" value={manual.txSignature}
