...
const POLL_INTERVAL_MS = 30_000; // fallback polling when no Realtime
...
function useCountdown(closeAt: string | null | undefined) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (diff <= 0) { setRemaining("Cloture"); return; }
      const d = Math.floor(diff / 86400000);
      const h = Math.floor((diff % 86400000) / 3600000);
      const m = Math.floor((diff % 3600000) / 60000);
      const s = Math.floor((diff % 60000) / 1000);
      if (d > 0) setRemaining(`${d}j ${h}h ${m}m`);
...
export function CryptoPriceChart({ ticker, priceTarget, marketCloseAt }: Props) {
  ...
  // Track whether Realtime has delivered at least one event
  const realtimeActiveRef = useRef(false);
  const countdown = useCountdown(marketCloseAt);
  const meta = COIN_META[ticker];
  const supabaseRef = useRef(getSupabase());

  // ── Helper: push a new price point ──────────────────────────────────────────
  ...
  // ── 1. Initial load: DB first, then klines proxy fallback ─────────────────
  useEffect(() => {
    setLoading(true);
    supabaseRef.current
      .from("crypto_prices")
      ...
      .then(({ data }) => {
        if (data && data.length > 0) {
          ...
          // Table empty → load klines via proxy
  ...
  // Starts after 35s; stops as soon as Realtime delivers an event.
  useEffect(() => {
    let pollId: ReturnType<typeof setInterval> | null = null;

    const poll = async () => {
      // If Realtime is working, stop polling
    ...
    // Start polling after 35s (give Realtime time to connect + deliver first event)
    ...
    <div className="rounded-2xl border border-border bg-card overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 pt-4 pb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground">{meta.symbol}USDT · Binance via Supabase</p>
        </div>
        <div className="flex flex-col items-end gap-0.5">
          {countdown && (
            <span className="text-[10px] text-muted-foreground">
              Cloture dans <span className="font-semibold text-foreground">{countdown}</span>
            </span>
          )}
          <span
      ...
      <div className="flex items-baseline gap-3 px-4 pb-3">
        ...
        {pctFromTarget != null && (
          ...
          >
            {isAbove ? "+" : ""}
            {pctFromTarget.toFixed(2)}% vs target
          </span>
        )}
      </div>
      ...
      <div className="mx-4 mb-3 flex items-center justify-between rounded-xl bg-muted/50 px-3 py-2">
        <span className="text-xs text-muted-foreground">Prix cible</span>
