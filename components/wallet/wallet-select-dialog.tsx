      ...
      <div className="w-full max-w-sm rounded-3xl border border-border bg-card p-5 shadow-2xl">
        ...
        {installed.length > 0 && (
          <div className="mb-4 space-y-2">
            {installed.map((w) => (
              <WalletItem key={w.type} wallet={w} onClick={() => onSelect(w.type)} />
            ))}
          </div>
        )}

        {/* Wallets non installés */}
...
function WalletItem({ wallet, onClick }: { wallet: WalletInfo; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-muted-foreground">
          {wallet.detected ? "Detected" : "Click to install"}
        </p>
      </div>
